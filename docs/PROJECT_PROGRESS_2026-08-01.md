...
原始数据共包含 83 个 CSV 文件、4,604,269 条记录，大小约 2.25 GiB。新版标签认证后，1,840,537 条通过当前监督规则，其中 1,186,857 条属于主 KD 亲和力任务；AbRank 等待按 measurement type 拆分，原始记录没有删除。
...
标签审核已经覆盖全部 83 个文件，并新增 A～D/NA 亲和力等级与独立任务头。旧兼容等级为 74 个 Gold、8 个 Silver、1 个 Auxiliary；主模型实际使用 `affinity_grade` 和 `primary_affinity_weight`。
...
- 修订后监督集合发现约 275,272 个不重复的完整抗体。
- 约 1,565,265 条记录属于完全相同抗体的重复出现。
- 约 352,151 条记录与其他数据组存在完全相同的抗体。
