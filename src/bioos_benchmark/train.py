...
from .features import AntibodyFeaturizer, InteractionFeaturizer
...
def temperature_sampling_weights(
    rows: list[dict[str, str]], alpha: float = 0.4, cap: int = 200_000,
    quality_field: str = "affinity_weight",
) -> np.ndarray:
    """Per-row sampling weights yielding dataset mass proportional to min(n, cap)**alpha."""
    if not 0 <= alpha <= 1:
    ...
    values=[]
    for row in rows:
        count=counts[row["source_file"]]
        try: quality=max(0.0,float(row.get(quality_field,"1") or 0))
        except ValueError: quality=0.0
        values.append((min(count,cap)**alpha/count)*quality)
    weights=np.asarray(values,dtype=np.float64)
    if not np.any(weights>0):raise ValueError("temperature sampler has no positive-weight rows")
    return weights/weights.mean()
...
def train_model(
    ...
    if featurizer_name == "baseline":
        featurizer = AntibodyFeaturizer()
    elif featurizer_name == "interaction":
        featurizer = InteractionFeaturizer()
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--featurizer", choices=("baseline", "interaction"), default="baseline")
    return parser
